...
# TEST-ONLY ISO overlay. Imports the real installer ISO unchanged (../iso.nix)
# and layers on top:
...
#
# The shipped `packages.iso` output is NOT affected — this is a separate
# `iso-test` package. The install logic under test (the baked ./installer scripts
# and the Kelvin module tree) is byte-for-byte the same as what ships.

{
  ...
  # Bake the test autoinstaller onto the ISO.
